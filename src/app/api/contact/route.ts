...
export async function POST(req: NextRequest) {
  try {
    ...
    const { firstName, lastName, email, org, interests, message } = body;
    ...
    const transporter = nodemailer.createTransport({
      ...
      secure: true, // SSL
      auth: {
        user: process.env.ZOHO_EMAIL,       // admin@akiraas.com
    ...
    // ── Email 1: notification to Akiraas ──────────────────────
    await transporter.sendMail({
      from: `"Akiraas Website" <${process.env.ZOHO_EMAIL}>`,
      to:   process.env.ZOHO_EMAIL,   // admin@akiraas.com
      replyTo: email,
    ...
    <table width="600" cellpadding="0" cellspacing="0" style="background:#FFFFFF;border:1px solid rgba(45,27,78,0.1);border-top:4px solid #2D1B4E;max-width:600px;width:100%;">
      ...
      <tr><td style="padding:28px 40px;">
        ...
        <div style="background:#FAF8F3;border-left:3px solid #B8963E;padding:18px 20px;">
          <p style="margin:0;font-size:14px;color:#3D3C38;line-height:1.8;white-space:pre-wrap;">${message.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</p>
        </div>
      </td></tr>

      <tr><td style="padding:0 40px 36px;">
        <a href="mailto:${email}?subject=Re: Your enquiry to Akiraas"
           style="display:inline-block;background:#2D1B4E;color:#FFFFFF;font-size:13px;font-weight:600;letter-spacing:0.04em;padding:12px 24px;text-decoration:none;border-radius:3px;">
          Reply to ${firstName} →
        </a>
      </td></tr>

      <tr><td style="padding:20px 40px;background:#F1EDE3;border-top:1px solid rgba(45,27,78,0.08);">
        <p style="margin:0;font-size:11px;color:#6B6A65;line-height:1.6;">
          Sent automatically from the contact form at <strong>akiraas.com</strong>.
          Reply to this email to reach ${fullName} directly at ${email}.
        </p>
      </td></tr>
  ...
  <tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" style="background:#FFFFFF;border:1px solid rgba(45,27,78,0.1);border-top:4px solid #B8963E;max-width:560px;width:100%;">
      ...
      <tr><td style="padding:32px 40px;">
        ...
        <p style="margin:0 0 16px;font-size:15px;color:#3D3C38;line-height:1.8;">
          Thank you for reaching out. We&rsquo;ve received your message and will get back to you within one business day.
        </p>
        <p style="margin:0 0 28px;font-size:15px;color:#3D3C38;line-height:1.8;">
          In the meantime, feel free to explore our communities at
          <a href="https://akiraas.com" style="color:#2D1B4E;font-weight:500;">akiraas.com</a>.
        </p>
        <div style="background:#FAF8F3;border-left:3px solid #B8963E;padding:16px 20px;margin-bottom:28px;">
          <p style="margin:0 0 8px;font-size:14px;color:#3D3C38;line-height:1.7;font-style:italic;">
            &ldquo;People don&rsquo;t just want to attend something &mdash; they want to belong to something.&rdquo;
          </p>
          <p style="margin:0;font-size:11px;color:#B8963E;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;">
            &mdash; GaneshKumar Murugan, Founder
          </p>
        </div>
        <p style="margin:0;font-size:14px;color:#6B6A65;line-height:1.8;">
          Warm regards,<br>
          <strong style="color:#2D1B4E;">The Akiraas Team</strong>
        </p>
      </td></tr>

      <tr><td style="padding:20px 40px;background:#F1EDE3;border-top:1px solid rgba(45,27,78,0.08);">
        <p style="margin:0;font-size:11px;color:#6B6A65;">
          Akiraas Pte Ltd &middot; Singapore &middot;
          <a href="https://akiraas.com" style="color:#2D1B4E;">akiraas.com</a> &middot;
          admin@akiraas.com
        </p>
      </td></tr>
